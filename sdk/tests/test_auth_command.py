...
def test_login_command_persists_workos_session(
    ...
    def fake_login(*, client_id: str, open_browser: bool) -> Tokens:
        captured["client_id"] = client_id
        captured["open_browser"] = open_browser
        return Tokens(access_token="AT", refresh_token="RT", user_id="u1", organization_id="org1")

    monkeypatch.setattr(auth_cli, "login_via_pkce", fake_login)

    result = CliRunner().invoke(
        auth_cli.app, ["login", "--base-url", "http://api.test", "--no-browser"]
    )
