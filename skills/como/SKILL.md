...
| **Record a research finding + evidence** (a value plus the proof behind it) onto a record or a list entry — you pick the command from where the attribute lives | **[references/records.md](references/records.md)** → "Recording a finding with evidence" (object attr → `records update --evidence-file`; list attr → `lists entry-data --evidence-file`) |
